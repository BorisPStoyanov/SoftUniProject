...
class PromotionManager
{
     ...
     * @return int
     */
    public function getMaxPromotion($category=null)
    {
        $promotion = 0;
        if($this->hasCategoryPromotion($category)){
            $promotion = $this->getCategoryPromotion($category);
        }

        $promotion = ($this->general_promotion > $promotion) ? $this->general_promotion : $promotion;
